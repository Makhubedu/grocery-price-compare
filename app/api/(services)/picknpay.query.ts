...
export async function queryPickNPay(searchString: string) {
    ...
    try {
        ...
        const productList = await page.evaluate(() => {
            const itemsList =Array.from(document.querySelectorAll('ui-product-grid-item'));

            return itemsList.map(item => {
                const itemData = item?.querySelector<any>('.product-grid-item')
                let promotionPrice = item.querySelector<any>('.product-grid-item__promotion-container')?.innerText
                ...
                return {
                    ...
                    stock: "",
                    product_image_url: item?.querySelector<any>('.product-grid-item > a cx-media > img')?.src
                }
            });
